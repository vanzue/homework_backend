...
class AzureTableStorage:
    ...
    def get_latest_id_by_partition(self, table_name: str, partition_key: str) -> int:
        ...
        try:
            ...
            if entities_list:
                # 找出最大的user_id
                max_user_id = max(int(entity[partition_key]) for entity in entities_list)
    ...
    def check_field_exists(self, table_name: str, field_name: str, field_value: Any) -> bool:
        table_client = self.table_service_client.get_table_client(table_name)
        try:
            # 构建查询过滤器
    ...
    def get_entity_by_field(self, table_name: str, field_name: str, field_value: Any) -> Optional[Dict[str, Any]]:
        table_client = self.table_service_client.get_table_client(table_name)
        try:
            # 构建查询过滤器
            filter_query = f"{field_name} eq '{field_value}'"
    ...
    def update_entity_fields(self, table_name: str, partition_key: str, row_key: str, fields_to_update: Dict[str, Any]) -> bool:
        table_client = self.table_service_client.get_table_client(table_name)
        try:
            # 获取实体
    ...
    def get_all_entities(self, table_name: str, page: int = 1, page_size: int = 10, **search_params) -> Tuple[List[Dict[str, Any]], int]:
        table_client = self.table_service_client.get_table_client(table_name)
        try:
            # 构建查询过滤器
            filter_query = []
